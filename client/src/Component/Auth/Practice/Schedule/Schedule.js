...
class Schedule extends Component {
      ...
      const list = schedule.schedule[key].map((each, j) => {
        ...
        return (
          <div key={'list' + j} className="each">
            ...
            <div className="times">
              <span className="time">{each.time.start}</span>
              <span className="while">～</span>
              <span className="time">{each.time.end}</span>
            </div>
  ...
  renderSource() {
    const link =
      this.props.loading || !this.props.source || this.props.source.length === 0 ? (
        <li>
        ...
        <div className={'schedule' + lib.pcClass(pc)}>
          <div className={'box schedule-next' + lib.pcClass(pc)}>
            <div className="title-frame">
              <label>次回の練習日</label>
              <div className="text">{showScheduleNext}</div>
            </div>
          </div>
          ...
          <div className={'box schedule-next' + lib.pcClass(pc)}>
            <div className="title-frame">
              <label>今後の練習日程</label>
              <div className="text">{showScheduleList}</div>
            </div>
          </div>
