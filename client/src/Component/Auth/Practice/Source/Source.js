    ...
    return item.contents.map((list, i) => {
      var ml = list.music.map((ml, j) => {
        ...
        var clickHandler =
          'audio' in data[ml]
            ? () => {
                this.props.sourcePlayRequest(item.id, data[ml].audio, true)
              }
            : () => {}
        var hasAudio =
      ...
      return (
        <li key={'l' + item.id + i}>
          <label
            className={'sticky-label' + (list.label.match(/第[0-9]部/) ? '' : ' other') + lib.pcClass(this.props.pc)}
          >
            {list.label}
          </label>
          <ol>{ml}</ol>
      ...
      <div className={'source' + lib.pcClass(this.props.pc)}>
        <div className={'contents-header' + lib.pcClass(this.props.pc)}>
          <div className="bread-navigation">
            ...
            <i className="fas fa-chevron-right"></i>
            <Link to="/practice/source">参考音源</Link>
          </div>
          <h2>参考音源</h2>
          <p>直近の演奏会の参考音源です</p>
        </div>
