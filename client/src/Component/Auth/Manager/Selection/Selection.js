...
class Selection extends Component {
  ...
  renderSearch() {
    const searchIcon = this.props.loadingSelectionListSearch ? (
        ...
        <div className="search-frame">
          <div className="search-box">
            ...
            <input
              ...
              onChange={(e) => this.props.changeSearchQuery(e.target.value)}
              ...
              placeholder="検索"
            />
            <div onClick={() => this.props.resetSearchQuery()} className={searchBarButtonClass}>
  ...
  renderList() {
    ...
    if (libManager.admin(this.props.user))
      console.log('/title/composer/arranger/duration/time/memo/like/created(GMT)/updated(GMT)')
    return this.props.selectionList.map((each, i) => {
      ...
      const link =
        selection.url.length > 0 && selection.url[0].match(/youtu\.?be/) ? (
      ...
      const edit =
        selection.postUserid === this.props.user._id && this.props.selectionPhase === 'getmusic' ? (
          <div className="edit">
      ...
      const contentClassEdit =
        selection.postUserid === this.props.user._id && this.props.selectionPhase === 'getmusic' ? ' add-edit' : ''
      const like =
        this.props.loadingSelectionLike || this.props.selectionPhase === 'getmusic' ? (
          false
        ) : (
          <div className="like">
            <span>{libManager.countLike(this.props.selectionLike, each._id)}</span>
          </div>
        )
      const duration =
        localStorage.getItem('selectionSort') === 'time' ? (
      ...
      if (libManager.admin(this.props.user))
        console.log(
          '/' +
            ...
            selection.memo +
            '/' +
            (this.props.loadingSelectionLike ? '-' : libManager.countLike(this.props.selectionLike, each._id)) +
            '/' +
            selection.createdAt +
            '/' +
            selection.updatedAt
      ...
      return (
        <Link
          key={each._id}
          to={'/manager/selection/detail/' + each._id}
        ...
        >
          <div className={'content' + contentClassLink + contentClassEdit}>
            ...
            <div className="composer-arranger">
              <span>
                <span>{composer}</span>
                {bar}
                <span>{arranger}</span>
  ...
  renderSort() {
    const postIcon =
      localStorage.getItem('selectionSort') === 'createdAt' ? (
        localStorage.getItem('selectionOrder') > 0 ? (
    ...
    const titleIcon =
      localStorage.getItem('selectionSort') === 'title' ? (
        localStorage.getItem('selectionOrder') > 0 ? (
          <i className="fas fa-sort-down"></i>
        ) : (
          <i className="fas fa-sort-up"></i>
    ...
    return (
      <div className="sort">
        <div className="buttons">
          <div className={'sort-created-at' + postClass} onClick={() => this.toggleSort('createdAt')}>
            {postIcon}投稿日時
          </div>
          <div className={'sort-title' + titleClass} onClick={() => this.toggleSort('title')}>
            {titleIcon}曲名
          </div>
          <div className={'sort-time' + timeClass} onClick={() => this.toggleSort('time')}>
            {timeIcon}演奏時間
          </div>
        </div>
      </div>
  ...
  renderPost() {
    const link =
      this.props.selectionPhase === 'getmusic' || libManager.admin(this.props.user) ? (
        <li className="selection-post">
          <Link to="/manager/selection/add">
            <div className="inner">
              <span>候補曲を投稿する</span>
              <Forward />
            </div>
    ...
    if (this.props.selectionPhase === 'onlyadmin') {
      ...
      return (
        <div className="text">
          <h2>候補曲の募集について</h2>
          <p>
            現在第33回定期演奏会に向けて候補曲を集めています。投稿数の制限はありませんので、思いついたときにどんどん投稿してください。
          </p>
          ...
          <p>
            {this.props.selectionList ? this.props.selectionList.length : '読み込み中'}
            {this.props.selectionList ? <span>件</span> : false}
          </p>
        </div>
      )
  ...
  render() {
    ...
    const endLabel = this.props.selectionList ? (
    ...
    return (
      <React.Fragment>
        ...
        <div className={'box selection-message' + lib.pcClass(this.props.pc)}>{showMessage}</div>
