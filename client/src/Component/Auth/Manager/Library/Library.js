...
export function makeLineUrl(array) {
  ...
  for (var i = 0; i < array.length; i++) {
    ...
    if (array[i].match(/^(https?|ftp)(:\/\/[-_.!~*\'()a-zA-Z0-9;\/?:\@&=+\$,%#]+)$/g)) {
      const label = array[i].match(/youtu\.?be/) ? (
      ...
      const link = (
        <a href={array[i]} target="_blank" rel="noopener noreferrer" className={className}>
          {label}
        </a>
      )
      newArray[i] = link
  ...
  const list = newArray.map((each, i) => (
    <div key={i} className="link-frame">
      {each}
    </div>
  ))
  return <div>{list}</div>
