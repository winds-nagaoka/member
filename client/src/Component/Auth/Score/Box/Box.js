...
class Box extends Component {
  ...
  renderBox () {
    if (this.props.loading || !this.props.boxList) return <div className="loading"><div className="loading1"></div><div className="loading2"></div><div className="loading3"></div></div>
    return this.props.boxList.map((each, i) => {
      ...
      return (
        <div key={'score-box' + each._id} className={disableClass} onTouchStart={() => {}} onClick={() => {this.props.setDisplayBoxModal(true, each)}}>
          <div className='label'><span>{each.label}</span></div>
          <i className='fas fa-archive'></i>
          <div className={locateClass}><span>{locate}</span></div>
        </div>
      )
    })
  }
    ...
    return (
      <React.Fragment>
        <div className={'contents-header' + lib.pcClass(this.props.pc)}>
          <div className='bread-navigation'><Link to='/'>ホーム</Link><i className="fas fa-chevron-right"></i><Link to='/score'>ウィンズスコア</Link><i className="fas fa-chevron-right"></i><Link to='/score/box'>楽譜管理箱</Link></div>
          <h2>楽譜管理箱</h2>
        ...
        <div className={'box score-box-add-link' + lib.pcClass(this.props.pc)}>
          <div className='send-button' onClick={() => this.addBox()}>{this.props.loadingAddBox ? <span><i className='fas fa-spinner fa-pulse'></i></span> : <span><i className='far fa-edit'></i>新しい箱を追加</span>}</div>
        </div>
