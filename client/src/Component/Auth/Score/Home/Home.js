...
class Home extends Component {
  ...
  renderScoreList() {
    if (this.props.loadingScore || !this.props.scoreList)
      return (
    ...
    return this.props.showList.map((each) => {
        ...
        >
          <div className="content">
            ...
            <div className="title-ja">
              <span>{each.titleJa}</span>
            </div>
            <div className="title-en">
              <span>{each.titleEn}</span>
            </div>
  ...
  renderLoadMore() {
    ...
    if (this.props.scoreList.length > 10 && this.props.scoreList.length !== this.props.showList.length) {
      return (
        <div onClick={() => this.props.loadMore()} className="score-more">
          <span>すべての楽譜を表示</span>
        </div>
      )
    }
  ...
  renderAddNewScore() {
    ...
    return (
      <div className={'box score-add-link' + lib.pcClass(this.props.pc)}>
        <div onClick={() => this.props.setDisplayEditScoreModal(true, 'new', undefined)}>
          {this.props.editPreLoading ? (
            ...
            <span>
              <i className="far fa-edit"></i>新しい楽譜を追加
            </span>
          )}
        </div>
      </div>
