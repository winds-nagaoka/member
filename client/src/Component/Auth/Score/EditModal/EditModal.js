...
class EditModal extends Component {
    ...
    return (
      <div className="radio-input">
        <input
          ...
          id="scoreStatusLend"
          value={2}
          checked={this.props.scoreEdit.scoreStatus === '2'}
        ...
        <input
          ...
          value={0}
          checked={this.props.scoreEdit.scoreStatus === '0'}
          onChange={(e) => this.selectChange('scoreStatus', e)}
        />
  ...
  renderLendInput() {
    if (this.props.scoreEdit.scoreStatus === '2') {
      return (
        <Input
          label="貸出先"
          value={this.props.scoreEdit.lendLocate}
          target="lendLocate"
          className="lend"
  ...
  renderBoxSelect() {
    const options = this.props.boxList.map((each) => {
      return (
        <option key={each._id} value={each.label}>
          {each.label} - {!each.locate ? '未設定' : each.locate}
        </option>
      )
    })
    return (
  ...
  renderInfo() {
    if (this.props.editPreLoading || !this.props.scoreEdit)
      return (
    ...
    return (
      <div className="radio-input">
        <input
          ...
          id="scoreTypeTrue"
          value={1}
          checked={this.props.scoreEdit.scoreType === '1'}
          onChange={(e) => this.selectChange('scoreType', e)}
        />
        ...
        <input
          ...
          id="scoreTypeFalse"
          value={0}
          checked={this.props.scoreEdit.scoreType === '0'}
          onChange={(e) => this.selectChange('scoreType', e)}
        />
  ...
  renderCopiedFromInput() {
    if (this.props.scoreEdit.scoreType === '1') {
      return (
        <Input
          label="コピーメモ"
          value={this.props.scoreEdit.copyMemo}
          target="copyMemo"
    ...
    return (
      <div className="radio-input">
        <input
          ...
          id="scoreLackTrue"
          value={1}
          checked={this.props.scoreEdit.scoreLack === '1'}
          onChange={(e) => this.selectChange('scoreLack', e)}
        />
        ...
        <input
          ...
          value={2}
          checked={this.props.scoreEdit.scoreLack === '2'}
          onChange={(e) => this.selectChange('scoreLack', e)}
        />
        ...
        <input
          ...
          id="scoreLackFalse"
          value={0}
          checked={this.props.scoreEdit.scoreLack === '0'}
          onChange={(e) => this.selectChange('scoreLack', e)}
        />
      ...
      const list = this.props.scoreEdit.lackList.map((each, i) => {
        return (
          <Input
            ...
            onChange={(e) => this.changeValueArray(i, e)}
          />
        )
      })
      return (
        <div>
          {list}
          <div className="add-data" onClick={() => this.addBlank('lackList')}>
    ...
    return (
      <div className="radio-input">
        <input
          ...
          id="scoreBasedTrue"
          value={1}
          checked={this.props.scoreEdit.scoreBased === '1'}
          onChange={(e) => this.selectChange('scoreBased', e)}
        />
        ...
        <input
          ...
          id="scoreBasedFalse"
          value={0}
          checked={this.props.scoreEdit.scoreBased === '0'}
          onChange={(e) => this.selectChange('scoreBased', e)}
        />
      ...
      <div className="score-edit-modal">
        <div className={'score-edit-modal-contents' + displayEditScoreModalClass + lib.pcClass(this.props.pc)}>
          <div className={'header' + lib.pcClass(this.props.pc)}>
            <div className="cancel" onClick={() => this.props.setDisplayEditScoreModal(false, undefined, undefined)}>
              キャンセル
            </div>
            <div className="title">{this.props.editMode === 'new' ? '新しい楽譜を追加' : '楽譜の修正'}</div>
            <div className="save" onClick={() => this.props.updateScoreEdit()}>
        ...
        <div
          className={'score-edit-modal-background' + displayEditScoreModalClass}
          onClick={() => this.props.setDisplayEditScoreModal(false, undefined, undefined)}
        ></div>
      </div>
    )
  }
