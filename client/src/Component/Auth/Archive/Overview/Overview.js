...
import {
  setNavigationTitle,
  setNavigationTitleArchiveConcertid,
  setBackNavigation,
...
class Overview extends Component {
    ...
    if (item.time['time'] && item.time['label']) {
      const time = (
        <div>
          ...
          <div>{item.time.time + item.time.label}</div>
        </div>
      )
    ...
    return item.contents.map((list, i) => {
      var ml = list.music.map((ml, j) => {
        var composer =
          'composer' in data[ml] ? (
            'arranger' in data[ml] ? (
              ...
              <span className="composer">{data[ml].composer}</span>
            )
          ) : 'arranger' in data[ml] ? (
            <span className="composer">{data[ml].arranger}編曲</span>
          ) : (
            ''
          )
        var additional =
        ...
        var clickHandler =
          'audio' in data[ml]
            ? () => {
                this.setAudio(data[ml].audio)
              }
            : () => {}
        var hasAudio =
          'audio' in data[ml] ? <i className="fas fa-play-circle"></i> : <i className="far fa-times-circle"></i>
        return (
          <li
            key={'m' + item.id + j}
            className={'track' + ('audio' in data[ml] ? ' has-audio' : '')}
      ...
      return (
        <li key={'l' + item.id + i}>
          <label className={'sticky-label' + (list.label.match(/第[0-9]部/) ? '' : ' other')}>{list.label}</label>
          <ol>{ml}</ol>
        </li>
      )
    })
  ...
  renderConcertNavigation() {
    ...
    const prevClass =
      'prev ' +
      libArchive.getPrevConcert(concertid, concertList) +
      ' ' +
      libArchive.getConcertType(concertid, concertList)
    const prevLink = libArchive.getPrevConcert(concertid, concertList) ? (
      <Link to={'/archive/overview/' + libArchive.getPrevConcert(concertid, concertList)} className={prevClass}>
    ...
    const nextClass =
      'next ' +
      libArchive.getNextConcert(concertid, concertList) +
      ' ' +
      libArchive.getConcertType(concertid, concertList)
    const nextLink = libArchive.getNextConcert(concertid, concertList) ? (
      <Link to={'/archive/overview/' + libArchive.getNextConcert(concertid, concertList)} className={nextClass}>
  ...
  renderNavigationLink() {
    if (this.props.loadingArchive || !this.props.concertList || !this.props.concertid) return
    const photoLink = !this.props.photoList ? (
      <li>
      ...
      <li>
        <Link to={'/archive/photo/' + this.props.concertid}>
          <div className="inner">
            <span>写真</span>
            <Forward />
          </div>
    ...
    const videoLink = !this.props.videoList ? (
      ...
      <li>
        <Link to={'/archive/video/' + this.props.concertid}>
          <div className="inner">
            <span>映像</span>
            <Forward />
          </div>
  ...
  renderConcertDetail() {
    if (this.props.loadingArchive || !this.props.concertList || !this.props.concertid)
      return (
    ...
    return (
      <div className="concert-detail">
        <div className="poster">{this.showPoster(item)}</div>
  ...
  renderBreadNavigation(loadingArchive, concertList, concertid) {
    if (loadingArchive || !concertList || !concertid)
      return (
    ...
    return (
      <div className="bread-navigation">
        ...
        <Link to="/archive">アーカイブ</Link>
        <i className="fas fa-chevron-right"></i>
        <Link to={'/archive/overview/' + concertid}>{libArchive.getConcertTitle(concertid, concertList)}</Link>
      </div>
    )
  }
      ...
      <React.Fragment>
        ...
        <div className={'archive-overview' + lib.pcClass(this.props.pc)}>
          <div className="article">
            ...
            <div className={'box' + lib.pcClass(this.props.pc)}>{this.renderConcertDetail()}</div>
          </div>
        </div>
